...
from scripts.dataset.dataset import HFDatasetWrapper, BEIRCorpusDataset
...
def evaluate_beir_dense(args, accelerator):
    ...
    for dataset in datasets:
        ...
        if args.do_search and accelerator.is_local_main_process:
            ...
            logger.info(f"retrieve metrics for {dataset_name}: NDCG@10={ndcg.get('NDCG@10', 0.0)}, Recall@100={recall.get('Recall@100', 0.0)}")
...
def evaluate_nano_beir_dense(args, accelerator):
    ...
    for dataset in datasets:
        ...
        if args.do_search and accelerator.is_local_main_process:
            ...
            logger.info(
                f"retrieve metrics for {dataset_name}: {ndcg, map_, recall, p}"
            )
            result["NDCG@10"].append(ndcg.get("NDCG@10", 0.0))
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str, required=True, help="HuggingFace model ID or path")
    parser.add_argument("--output_dir", type=str, default="output_beir_dense", help="Output directory")
    ...
    parser.add_argument("--beir_datasets", type=str, default=default_beir_datasets, help="Comma separated BEIR datasets")
    parser.add_argument("--nano_beir_datasets", type=str, default=default_nano_beir_datasets, help="Comma separated NanoBEIR datasets")
    parser.add_argument("--eval_nano", action="store_true", help="Evaluate on NanoBEIR")
    parser.add_argument("--no_eval_beir", action="store_true", help="Skip BEIR evaluation")
    ...
    accelerator = Accelerator()
    logger.info(f"Using device: {accelerator.device}, Process: {accelerator.process_index}/{accelerator.num_processes}")
